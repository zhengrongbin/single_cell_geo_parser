...
def sync_samples_from_gsm_factor(infile, gse_col, factor_col, fsave, xmlPath='geo', fill_or_not = False, refresh = False):
    ...
    for iterm in need_added_samples:
        ...
        getType = getGEOSamples_byType_gse.getGeoSamples_byTypes(path = "repository_samples.pickle", datatype = ['sc-rna-seq', 'sc-atac-seq'],
            gsmids=[gseid], refresh=refresh, ddir = xmlPath)
        if getType:
...
def getLocalGeo(fsave, fill_or_not=False, xmlPath="geo", DataType=False, refresh = False):
    ...
    for gseid in local_repo_samples:
        print(gseid)
        getType = getGEOSamples_byType_gse.getGeoSamples_byTypes(path = "repository_samples.pickle", datatype = ['sc-rna-seq', 'sc-atac-seq'], gsmids=[gseid], refresh=refresh)
        if getType:
            for i in getType.keys():
                # parse sample annotation
                list_sample = scrna_parser_detail_gse.update_one_sample(gseid=gseid, ddir=xmlPath)
